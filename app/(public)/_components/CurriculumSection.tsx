      ...
      <div className="flex w-full max-w-[1000px] flex-col items-center gap-10">
        ...
        >
          ...
          <h2 className="text-foreground font-monument text-3xl font-normal tracking-wide sm:text-4xl md:text-5xl lg:text-6xl">
            <span className="text-[#FE6168]">A specially designed, Curriculum</span>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {curriculumItems.map((item, index) => (
                  <motion.div
                    ...
                    transition={{ delay: index * 0.05, ease: "easeInOut", duration: 1 }}
                    className="bg-card/30 border-border flex items-center gap-4 rounded-2xl border p-5 backdrop-blur-sm transition-all hover:border-[#FE6168]/50 hover:bg-card/60"
                  >
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#FE6168]/10 text-[#FE6168]">
                      <span className="font-monument text-sm">{index + 1}</span>
                    </div>
                    <span className="text-foreground font-mont text-sm font-medium leading-tight">
                      {item}
                    </span>
                  </motion.div>
                ))}
